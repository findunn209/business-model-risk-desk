  ...
  failure_modes: [
    {
      ...
      how_the_model_breaks:
        "On this sample the operator takes the homeowner’s card, pays the worker later, and has not built the processor-required provider controls. If you charge the homeowner on your brand, pay the provider before confirmed fulfillment, and those controls are not in the file, you are unfunded merchant of record for unfulfilled GMV (the full amount the homeowner paid) for N days. Processors can onboard that funds flow. Stripe, Adyen, and PayPal sell “customers pay you, you pay sellers.” They fail it, or tightly condition it, when the controls are missing: KYC on the worker/provider (the person selling the service on the marketplace), a check they can actually do the advertised work here, and post-job ratings plus customer confirmation of quality so the bench stays reliable. The break is those unasked controls. The 18% take is economics. 100% of GMV is the card, chargeback, refund, and deposit exposure. Exposure is unfulfilled GMV from charge to job done, then the dispute tail. Same logic as subscriptions. A monthly charge is about a month of exposure; an annual charge is about a year. Prepaid hours, memberships, or job credits are money now for work later — still exposure until the work is done. Cash on hand must cover that exposure at any time; as volume grows, exposure grows. Instant payout before the job is confirmed is how platform cash gets eaten, not why a merchant account is impossible.",
      time_to_break: "this_cycle",
      evidence: "inferred",
      evidence_note:
  ...
  tripwire_sections: [
    {
      ...
      items: [
        ...
        },
        {
          ...
          body: "Two stacks. What the PSP requires of you: identity, tax, bank, a site that says delivery and refunds, and an honest funds flow — take the card and pay workers, or a lead fee only. What they will not do for you, and what you must bring: KYC on each provider (sub-merchant in a PSP underwrite), proof they can do the advertised job, pause payout until delivered, customer confirmation of quality, and ratings you own. Exposure is days between charge and fulfillment times volume — same for monthly vs annual prepaid. Cash covers it. Tools if you pay providers: Stripe Connect, or a PSP plus a payouts rail (Hyperwallet, Payoneer, and the like). A payout rail KYCs payees and sends money; it does not move card risk if you already charged the homeowner.",
        },
        {
          lead: "What fails",
